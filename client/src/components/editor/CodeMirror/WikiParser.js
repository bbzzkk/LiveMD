...
export default class WikiParser {
  ...
  static isInside(position, region) {
    if (position.line < region.start.line || region.end.line < position.line)
      return false;
    if (
      position.line === region.start.line &&
      position.column < region.start.column
    )
      return false;
    if (
      position.line === region.end.line &&
      position.column > region.end.column
    )
      return false;
    return true;
  }
  ...
  static convertToCustomHast(hast) {
    ...
    //   const language =
    //     (htmlClasses &&
    //       htmlClasses[0].substring(9 /* length of 'language-' */)) ||
    //     null;
    //   if (language in Apps) {
  ...
  static renderCustomHast(customHast, ctx = {}) {
    function h(name, properties, children) {
      ...
      //   if (
      //     ctx.cursorPosition &&
      //     WikiParser.isInside(ctx.cursorPosition, position)
      //   ) {
      //     active = true;
      //   }
      //   if (appComponent) {
      ...
      if (name === 'a' && internalLink.test(properties.href)) {
        ...
        propsForLink.className = propsForLink.className
      ...
      if (propsForElem) {
        propsForElem.className = propsForElem.className
  ...
  static getExtraFencingChars(startFencingStr, appText) {
    ...
    const symbolCounter = new RegExp(
      `^\x20*(${fencingChar}{${originalFencing.length},})`,
    );
   ...
   * @returns {string}
   */
  static replaceAppCode(
    originalText,
    originalAppLocation,
    appLanguage,
    newAppText,
  ) {
    const textBefore = originalText.substring(
      0,
      originalAppLocation.start.offset,
    );
    const textAfter = originalText.substring(originalAppLocation.end.offset);
    const fencedText = originalText.substring(
      originalAppLocation.start.offset,
      originalAppLocation.end.offset,
    );
    const startFencedStr = fencedText.substring(
      0,
      fencedText.search(/\r\n|\r|\n/),
    );
    const [backticks, originalBackticks] = WikiParser.getExtraFencingChars(
      startFencedStr,
      newAppText,
    );
